    ...
    def patch(self, request):
        ...
        if user is not None:
            ...
            serializer = UserSerializer(
                instance=queryset, data=request_data, partial=True
            )
            if serializer.is_valid():
                new = serializer.save()
                new.set_password(new.password)
...
class JobApplications(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            mail = EmailMessage(
                subject, contact_message, from_email, ["hr@bridgefix.co"]
            )
...
class JobList(viewsets.ModelViewSet):
    queryset = Job.objects.filter(job_status="ACTIVE")
...
class ContectForm(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            email = request.data.get("email")
            contact_no = request.data.get("phone_number")
            massage = request.data.get("message")
            ...
            pdb.set_trace()
            contact_message = "%s %s %s %s %s %s %s %s" % (
                "Full name :",
                Name,
                "\n" "Contaact no. :",
                contact_no,
